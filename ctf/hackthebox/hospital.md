...
![make an account](.gitbook/img/htb/hospital/make_an_account.png)
...
![request from upload.php](.gitbook/img/htb/hospital/request_1.png)
...
![uploading p0wny](.gitbook/img/htb/hospital/uploading_p0wny.png)
...
![fuzzing uploads](.gitbook/img/htb/hospital/ffuf_uploads.png)
...
![p0wny shell on linux server](.gitbook/img/htb/hospital/p0wny_shell_1.png)
...
![ifconfig output](.gitbook/img/htb/hospital/ifconfig_ouptut_1.png)
...
```
![privesc on LS](.gitbook/img/htb/hospital/privesc_linux_server_2.png)

Upon achieving root access, the /etc/shadow file is accessible, allowing for the extraction of hashes for cracking attempts.

![reading /etc/shadow](.gitbook/img/htb/hospital/reading_shadow.png)
...
![drwilliams webmail](.gitbook/img/htb/hospital/drwilliams_webmail_1.png)
...
![email from drbrown](.gitbook/img/htb/hospital/email_from_drbrown_1.png)
...
![running the payload](.gitbook/img/htb/hospital/curl_nc_ghostscript.png)

Access the compose function, attach the malicious .eps file, set the recipient as Drbrown, and proceed to send the email.

![composing email](.gitbook/img/htb/hospital/composing_email_1.png)
...
![received "get" hit](.gitbook/img/htb/hospital/get_hit_1.png)
...
```
![ghostscript reverse shell injection](.gitbook/img/htb/hospital/ghostscript_reverse_shell.png)

![composing another email](.gitbook/img/htb/hospital/composing_email_2.png)

A reverse shell connection is established, granting the attacker access to the victim machine.

![ps shell received](.gitbook/img/htb/hospital/recv_ps_shell_1.png)

The user flag is at __C:\Users\drbrown.HOSPITAL\Desktop__

![shell as drbrown](.gitbook/img/htb/hospital/whoami_drbrown_1.png)
...
![serving p0wny](.gitbook/img/htb/hospital/serving_p0wny_1.png)

downloading the p0wny shell in the htdocs directory


![saving p0wny](.gitbook/img/htb/hospital/saving_p0wny_htdocs.png)

Navigate to https://hospital.htb/pwny.php and a shell as nt authority will be given

![whoami admin](.gitbook/img/htb/hospital/whoami_admin_1.png)

### Solution 2 RDP

There is a password in the ghostscript.bat file. __drbrown:chr!$br0wn__

![drbrown pass](.gitbook/img/htb/hospital/drbrown_pass_1.png)
...
![admin pass](.gitbook/img/htb/hospital/admin_pass_1.png)

The credentials successfully authenticate with evil-winrm, granting admin access to the victim machine.

![winrm as admin](.gitbook/img/htb/hospital/winrm_as_admin.png)
